    ...
    public static void main(String argv[]) {
        ...
        */
        ...
        Company oldest = companies.stream()
                .min((c1, c2) -> Integer.compare(c1.getFoundationYear(), c2.getFoundationYear())).get();
        ...
        Person p99 = companies.stream()
                .map(Company::getPersonList)
                .flatMap(List::stream)
                .reduce(new Person(), (per1, per2) -> {
                    per1.setName(per1.getName() + per2.getName());
                    return per1;
                });
        ...
        Integer sumOf1000Numbers = Stream.iterate(0, x -> x + 2).limit(1000).reduce(0, (n1, n2) -> n1 + n2);
        ...
        startTime = System.nanoTime();
        sumOf1000Numbers = Stream.iterate(0, x -> x + 2).limit(1000).parallel().reduce(0, (n1, n2) -> n1 + n2);
        endTime = System.nanoTime();
        duration = (endTime - startTime);
